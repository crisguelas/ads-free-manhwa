...
function LatestUpdateBlock({ series }: LatestUpdateBlockProps) {
  ...
  const line3 =
    series.genres.length > 0 ? series.genres.slice(0, 4).join(" · ") : "Chapters & reader";
...
type BrowseHeaderSearchProps = {
  /** Anchor id to scroll into view when the field is focused (e.g. catalog grid). */
  browseTargetId: string;
};

/**
 * Header search field: focuses the browse/catalog area so users land on titles without a backend search index yet.
 */
export function BrowseHeaderSearch({ browseTargetId }: BrowseHeaderSearchProps) {
  const focusBrowse = useCallback(() => {
    document.getElementById(browseTargetId)?.scrollIntoView({ behavior: "smooth", block: "start" });
  }, [browseTargetId]);

  return (
    <label className="relative hidden min-w-[200px] max-w-sm flex-1 md:flex">
